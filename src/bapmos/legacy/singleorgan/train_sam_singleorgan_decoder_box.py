"""SAM decoder-only fine-tuning for one organ using a single box prompt per slice (binary segmentation)."""
...
class SAMSingleOrganBoxTrainer:
    def __init__(self, config):
        ...
        log_baseline_taxonomy_startup(self._taxonomy, prefix=f"SAMSingleOrganBoxTrainer({self.organ})")
    ...
    def loss_fn(self, logits, gt_bin):
        ...
        return ce_loss + torch.stack(dice_losses).mean()
    ...
    def _box_prompt(self, mask_bin, original_size, sample_id, is_train: bool):
        ...
        return torch.as_tensor(boxes_t, device=self.device, dtype=torch.float32)
    ...
    def _forward_one(self, image_rgb, mask_bin, filename, is_train: bool):
        ...
        boxes_t = self._box_prompt(mask_bin, original_size, filename, is_train)
        if boxes_t is None:
            return None
        box_i = boxes_t[0:1, :]
        with torch.no_grad():
            sparse, dense = self.model.prompt_encoder(
                points=None, boxes=box_i.unsqueeze(1), masks=None
            )
...
def main():
    ...
    p.add_argument("--organ", type=str, required=True, help="Organ key for this dataset taxonomy")
    ...
    p.add_argument("--box_jitter_px", type=int, default=None, help="BAP-MOS train box jitter (0..N px per edge).")
    p.add_argument("--box_noise_pixels", type=int, default=10, help="Legacy alias for --box_jitter_px.")
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Default: runs/SingleOrgan/Baseline/<organ>_box"
        ),
    )
    p.add_argument(
        "--resume",
    ...
    default_run_root = Path("runs") / "SingleOrgan" / "Baseline" / f"{args.organ}_box"
    ...
    if resume_arg:
        ...
        run_name = args.run_name or f"{args.organ}_single_box_{ts}"
    ...
    trainer = SAMSingleOrganBoxTrainer(config)
