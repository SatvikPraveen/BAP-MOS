"""
Taxonomy-aware SAM decoder-only fine-tuning with per-organ point prompts.
...
class SAMMultiOrganTrainer:
    def __init__(self, config):
        ...
        log_baseline_taxonomy_startup(self._taxonomy, prefix="SAMMultiOrganPointsTrainer")
    ...
    def _points_prompt(self, mask_multi, original_size, sample_id, is_train: bool):
        """Per-organ positive + ring-negative point prompts from GT."""
        base = self.cfg["seed"]
        ...
        point_sets = sample_per_organ_points_with_negatives(
            mask_multi,
            num_pos_per_organ=self.cfg.get("num_pos_per_organ", 1),
            num_neg=self.cfg.get("num_neg_points", 3),
            ring_width=self.cfg.get("ring_width", 20),
            rng=rng,
            organ_to_class=self._taxonomy.organ_to_class,
        )

        valid_point_sets = []
        present_organs = []
        for organ_name in self._taxonomy.organ_keys:
            if point_sets.get(organ_name) is not None:
    ...
    def _forward_one(self, image_rgb, mask_multi, filename, is_train: bool):
        """Forward pass for one sample with per-organ point prompts."""
        ...
        prompt_result = self._points_prompt(mask_multi, original_size, str(filename), is_train=is_train)
    ...
    def train_loop(self, train_loader, val_loader):
        ...
        for epoch in range(start_epoch, self.cfg["max_epochs"]):
            ...
            val_hd95 = (
                ...
                else (val_summ.get("val_hd95") if val_summ else None)
            )
            val_rep_dice = (
                ckpt_scores.organ_balanced_dice
                if ckpt_scores is not None
                else (val_summ.get("val_dice") if val_summ else None)
            )
            if val_msd is None and val_summ:
    ...
    def evaluate_test_once(self, test_loader):
        ...
        write_method_evaluation_meta(
            ...
            split="test",
            extra={"baseline": "multiorgan_sam", "prompt_type": "points"},
        )
...
def main():
    ...
    parser.add_argument("--num_pos_per_organ", type=int, default=1)
    parser.add_argument("--num_neg_points", type=int, default=3)
    parser.add_argument("--ring_width", type=int, default=20)
    ...
    parser.add_argument(
        "--cosine_lr",
        action="store_true",
        help="Use CosineAnnealingLR instead of flat LR (default: flat, matches box / UNet baselines).",
    )
    parser.add_argument(
        "--splits_subdir",
        type=str,
    ...
    if resume_arg:
        ...
        if not sam_resolved.is_file():
            raise FileNotFoundError(
                f"SAM checkpoint not found: {sam_ckpt!r} (tried under {root}/models)"
            )
        ...
        run_name = args.run_name or f"{bundle}_multiorgan_decoder_points_realdata_{timestamp}"
        ...
        config = {
            ...
            "dataset": bundle,
            "baseline": "sam_multiorgan_points",
    ...
    # Non-method SAM points baseline: regional CE+Dice only (never Kervadec).
    ...
    print("\n" + "="*60)
    print(f"Multi-Organ SAM Training (Point Prompts) — bundle={bundle}")
    print("="*60)
